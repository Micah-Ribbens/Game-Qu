...
class Function(ABC):
    ...
    def get_x_coordinate(self, y_coordinate):
        """:returns: float; the x coordinate associated with that y coordinate (only works if the function is invertible)"""

        return self.get_inverted_function().get_x_coordinate(y_coordinate)

    def get_inverted_function(self):
        """ :returns: Function; the inversion function of this current function. By default it raises a ValueError because not all functions will have an inverse"""
    ...
    def get_new_function(get_y_coordinate_method):
        """:returns: Function; a function with the get_y_coordinate method being the provided 'get_y_coordinate_method'"""
