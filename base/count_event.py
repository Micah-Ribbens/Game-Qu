...
class CountEvent:
    ...
    def __init__(self, count_needed):
        """ Initializes the object
            :parameter count_needed: int; the number of times this event must happen to be done"""
    ...
    def get_is_active(self):
        """ :returns: boolean; whether the function is active. If it is not active, all functions that return a boolean
    ...
    def current_count_is_positive(self):
        """:returns: boolean; whether the positive (count > 0)"""

        return self.current_count > 0 and self.is_active

    def current_count_is_negative(self):
        """:returns: boolean; whether the count is negative (count < 0)"""

        return self.current_count < 0 and self.is_active
    
    def get_count_needed(self):
        """:returns: int; the number of times this event must happen to be done"""

        return self.count_needed

    def get_current_count(self):
        """:returns: int; the current count of this event"""
